<x-guest-layout>
    <form method="POST" action="{{ route('creategroup') }}">
        ...
        <!-- Name -->
        <div>
            <x-required-input for="name" :value="__('Group Name')" />
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-required-input for="supervisor" :value="__('Supervisor')" />
        ...
        <div class="mt-4">
            <x-required-input for="groupmembers" :value="__('Group Members')" />
